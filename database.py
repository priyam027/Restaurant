...
ALLOWED_TABLES = {'categories', 'food', 'offers', 'bookings', 'reviews', 'admin_users', 'contact_messages'}
ALLOWED_COLUMNS = {'description', 'price', 'image', 'badge', 'role'}
ALLOWED_TYPES = {'TEXT', 'REAL', 'INTEGER'}

DEFAULT_ADMIN_USERNAME = 'admin'
DEFAULT_ADMIN_PASS = 'Admin_@123'
DEFAULT_IMAGE_PATH = 'image/indian.jpg.png'
...
def _add_column_if_missing(conn, table, column, coltype):
    ...
    if table not in ALLOWED_TABLES or column not in ALLOWED_COLUMNS or coltype not in ALLOWED_TYPES:
        raise ValueError("Invalid identifier or column type for table migration.")
    ...
    with get_db() as conn:
        ...
        if not existing_admin:
            conn.execute(
                ...
                (DEFAULT_ADMIN_USERNAME, generate_password_hash(DEFAULT_ADMIN_PASS))
            )
            conn.commit()
            logger.info("Default admin created -> username: admin | password: Admin_@123")
        ...
        if not conn.execute('SELECT id FROM categories LIMIT 1').fetchone():
            conn.executemany(
                ...
                [
                    ('Paneer Handi', 'Soft paneer cubes cooked in a rich, creamy tomato gravy with aromatic Indian spices.', 20.00, DEFAULT_IMAGE_PATH),
                    ...
                    ('Indian', 'Aromatic spices simmered low and slow.', 10.75, DEFAULT_IMAGE_PATH),
...
if __name__ == '__main__':
    init_db()
    logger.info("Database ready: restaurant.db")
